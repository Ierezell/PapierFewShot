...
for i_epoch in range(NB_EPOCHS):
    for i_batch, batch in enumerate(train_loader):
        ...
        if i_batch % 3 == 0 or i_batch % 3 == 1:
            ...
            writer.add_scalar("Loss_dsc", lossDsc,
            ...
            writer.add_scalar("lossCnt", lossCnt,
                              global_step=i_batch+len(train_loader)*i_epoch)
            writer.add_scalar("lossMch", lossMch,
                              global_step=i_batch+len(train_loader)*i_epoch)
            writer.add_scalar("lossAdv", lossAdv,
                              global_step=i_batch+len(train_loader)*i_epoch)
            writer.add_scalar("LossTot", lossCnt+lossMch+lossAdv,
                              global_step=i_batch+len(train_loader)*i_epoch)
        if i_batch % PRINT_EVERY == 0:
            ...
            grid = torchvision.utils.make_grid(
                images_to_grid, padding=4, nrow=2, scale_each=True
            )
